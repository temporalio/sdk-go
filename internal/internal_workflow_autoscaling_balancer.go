...
// waitForKind keeps a blocked turn out of its poller target.
func (a *workflowAutoscalingBalancer) waitForKind(
	...
	for {
		...
		_, ok := a.admissionCandidates(kind, snapshot.weights)
		if ok {
			a.mu.Unlock()
			return nil
		}
...
func (a *workflowAutoscalingBalancer) tryAcquireLocked(
	...
	candidates, ok := a.admissionCandidates(kind, snapshot.weights)
	if !ok {
...
func (a *workflowAutoscalingBalancer) admissionCandidates(
	kind enumspb.TaskQueueKind,
	weights map[string]float32,
) (map[string]float32, bool) {
	if len(weights) == 0 {
	...
	missing := a.coverageCandidates(kind, weights)
	otherMissing := a.coverageCandidates(other, weights)
	if len(missing) > 0 {
	...
	sticky := a.stickyCandidates(weights)
