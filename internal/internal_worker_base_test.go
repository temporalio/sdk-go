...
// TestSaturationScaleUp_DeadlockCase verifies the fix for the throughput gate
// deadlock: 1 saturated poller can scale up even though throughput is flat.
// The saturation budget allows a bounded probe, and throughput growth from the
// new poller resets the budget for further scaling.
func (s *PollScalerReportHandleSuite) TestSaturationScaleUp_DeadlockCase() {
	ps := newPollScalerReportHandle(pollScalerReportHandleOptions{
...
func (s *PollScalerReportHandleSuite) TestSaturationScaleUp_IdlePollers() {
	...
	// Period 2: pollers are idle (empty polls present). Budget should be cleared.
	for range 15 {
		ps.handleTask(newTestTask(0))
	}
	for range 5 {
	...
	// +1 blocked: not saturated, budget cleared.
	ps.handleTask(newTestTask(1))
	require.Equal(s.T(), 1, int(ps.target.Load()),
		"scale-up should be blocked when pollers have spare capacity")
}
