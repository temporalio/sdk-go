...
func signalWithStartWorkflow(ctx internal.Context, request SignalWithStartWorkflowRequest) (*SignalWithStartWorkflowResponse, error) {
	c := internal.NewNexusClient(Endpoint, ServiceName)
	fut := c.ExecuteOperation(ctx, SignalWithStartWorkflowOp, request.ToProto(), internal.NexusOperationOptions{})
