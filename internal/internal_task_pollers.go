...
func (wtp *workflowTaskProcessor) processWorkflowTask(task *workflowTask) (retErr error) {
	...
	if err := visitProtoPayloads(ctx, wtp.inboundPayloadVisitor, task.task); err != nil {
		...
		failReq := wtp.errorToFailWorkflowTask(task.task.TaskToken, task.task.WorkflowExecution.GetWorkflowId(), err)
...
func (wtp *workflowTaskProcessor) RespondTaskCompletedWithMetrics(
	...
	if err = visitProtoPayloads(ctx, wtp.outboundPayloadVisitor, taskCompletion.rawRequest); err != nil {
		...
		failReq := wtp.errorToFailWorkflowTask(task.TaskToken, task.WorkflowExecution.GetWorkflowId(), err)
		var submitErr error
