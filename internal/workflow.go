...
// system endpoint names for Nexus endpoint registration/routing.
const systemNexusEndpoint = "temporal-system"
...
//
// Exposed as: [go.temporal.io/sdk/workflow.SignalWithStartWorkflow]
func SignalWithStartWorkflow(
	...
	if options.ID != "" && options.ID != workflowID {
		future, settable := NewFuture(ctx)
		settable.Set(nil, fmt.Errorf("workflow ID from options not used, must be unset or match workflow ID parameter"))
		return future
	}
	...
	ctx = workflowContextWithNewHeader(ctx)
	return i.SignalWithStartWorkflow(ctx, options.ID, signalName, signalArg, options, workflowType, workflowArgs...)
}

func (wc *workflowEnvironmentInterceptor) SignalWithStartWorkflow(
	ctx Context,
	workflowID string,
	signalName string,
	signalArg interface{},
	...
	Go(ctx1, func(ctx Context) {
		...
		settable.Set(WorkflowExecution{ID: workflowID, RunID: result.RunID}, nil)
	})
	return future
}
