...
func (wc *workflowEnvironmentInterceptor) ExecuteActivity(ctx Context, typeName string, args ...interface{}) Future {
	...
	if err != nil {
		// Mark the coroutine as panicking before raising the panic. This
		// prevents defers (ex: CompleteSession) from yielding the coroutine
		// via Future.Get(), which would freeze the panic unwinding and allow
		// the current WFT to complete with commands generated by the defer.
		getState(ctx).panickedValue = err
		panic(err)
	}
