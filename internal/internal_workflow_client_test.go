...
func (s *workflowClientTestSuite) TestStartWorkflow() {
	...
	s.Equal(makeTransferAware(converter.GetDefaultDataConverter()), client.dataConverter)
	s.Nil(err)
...
func (s *workflowClientTestSuite) TestExecuteWorkflowWithDataConverter() {
	...
	s.Equal(makeTransferAware(iconverter.NewTestDataConverter()), client.dataConverter)
	s.Nil(err)
	s.Equal(createResponse.GetRunId(), resp.GetRunID())
}
