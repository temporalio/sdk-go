...
func loadConfig() (config, error) {
	cfg := config{
		...
		workflowFile: valueOrDefault(os.Getenv("CI_HISTORY_WORKFLOW_FILE"), defaultWorkflowFile),
		event:        valueOrDefault(os.Getenv("CI_HISTORY_EVENT"), defaultEvent),
		serverURL:    valueOrDefault(os.Getenv("GITHUB_SERVER_URL"), "https://github.com"),
...
func run(ctx context.Context, cfg config) error {
	...
	return appendSummary(cfg.summaryPath, renderSummary(cfg, jobs))
}
...
func fetchRunJobs(ctx context.Context, client apiClient, run workflowRun) ([]workflowJob, error) {
	...
	for page := 1; ; page++ {
		...
		for _, job := range response.Jobs {
			...
			job.RunUpdatedAt = valueOrDefault(run.UpdatedAt, run.CreatedAt)
...
func renderSummary(cfg config, jobs []workflowJob) string {
	var b strings.Builder
	fmt.Fprintf(&b, "## CI History for %s\n\n", cfg.branch)
	b.WriteString("## Job History\n\n")
	b.WriteString(renderJobStatsTable(jobs))
	b.WriteString("\n\n## Most Failure-Prone Jobs, 30 days\n\n")
	b.WriteString(renderMostFailureProneJobs(jobs))
	b.WriteString("\n\n## Recent Failed Jobs\n\n")
...
func renderJobStatsTable(jobs []workflowJob) string {
	rows := buildJobSummaries(jobs, time.Now().UTC())
	if len(rows) == 0 {
...
func renderMostFailureProneJobs(jobs []workflowJob) string {
	rows := buildJobSummaries(jobs, time.Now().UTC())
	rows = filter(rows, func(row jobSummary) bool {
		return row.ThirtyFailures > 0
	})
...
func completedAt(job workflowJob) string {
	...
	if job.RunUpdatedAt != "" {
		return job.RunUpdatedAt
	}
	return job.RunCreatedAt
}
...
func hasFailure(jobs []workflowJob) bool {
	for _, job := range jobs {
		if failed(job) {
			return true
		}
