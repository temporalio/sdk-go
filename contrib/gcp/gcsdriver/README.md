...
Package `go.temporal.io/sdk/contrib/gcp/gcsdriver` provides a GCS-backed [`converter.StorageDriver`](https://pkg.go.dev/go.temporal.io/sdk/converter#StorageDriver) for the Temporal Go SDK's [external storage](https://pkg.go.dev/go.temporal.io/sdk/converter#ExternalStorage) system. Large payloads are offloaded to Google Cloud Storage and replaced with a storage reference in the Temporal history event; the reference is resolved back to the original payload before it reaches application code.

## Usage

The `go.temporal.io/sdk/contrib/gcp/gcsdriver` package defines the driver and its configuration. Use the companion package `go.temporal.io/sdk/contrib/gcp/gcsdriver/gcssdk` to wrap a Cloud Storage client.
...
Characters outside GCS's safe set are percent-encoded per byte of their UTF-8 representation. GCS forbids carriage-return, line-feed, and the literal segments `.` and `..`, and strongly recommends avoiding `# [ ] * ? : " < > |` and control characters; those characters plus `/` and `%` are encoded, leaving all other Unicode intact. Empty segments are replaced with `null`. See the [GCS object naming documentation](https://cloud.google.com/storage/docs/objects#naming) for details.
...
The driver creates objects but never deletes them. Without a cleanup policy the bucket will grow indefinitely and accrue unbounded storage costs. Configure a [GCS Object Lifecycle Management](https://cloud.google.com/storage/docs/lifecycle) rule on the bucket to automatically delete objects older than your maximum workflow run timeout plus your namespace retention period. See [External Storage Lifecycle](https://docs.temporal.io/external-storage#lifecycle) for details on determining the appropriate retention window.
