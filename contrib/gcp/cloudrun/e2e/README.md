# Google Cloud Run OpenTelemetry end-to-end harness
...
Build the container image with the provided `cloudbuild.yaml` from the repository root:

```bash
gcloud builds submit --config=contrib/gcp/cloudrun/e2e/cloudbuild.yaml \
  --substitutions=_IMAGE=<region>-docker.pkg.dev/<project>/<repo>/sdk-go-otel-e2e:latest .
```
