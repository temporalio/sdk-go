...
- Initial (experimental) release of the Cloud Run integration. `Plugin` is a client
  ...
  metadata server, and fails fast with a clear error when the process is not running on
  ...
  injection into the plugin via `PluginOptions`. Unlike `contrib/aws/lambdaworker`, this
  is a plugin rather than a worker wrapper, because Cloud Run runs a long-lived container.
