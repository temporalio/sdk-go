...
- **Name** — the Cloud Run worker pool name (from the `CLOUD_RUN_WORKER_POOL` environment variable
  on a worker pool), falling back to the service name (`K_SERVICE`) on a Cloud Run service.
- **Revision** — from `CLOUD_RUN_REVISION` (worker pools), falling back to `K_REVISION` (services).
  It forms the main part of the worker identity.
...
The plugin then applies the metadata on the **client**: it sets `Identity` to
`"<instanceID>@<revision>"` (falling back to `"<instanceID>@<name>"`, or just the instance ID) unless
